...
class ExtRandomHorizontalFlip(object):
    ...
    def __call__(self, img, lbl, edge):
        """
        Args:
        ...
        if random.random() < self.p:
            ...
            return F.hflip(img), F.hflip(lbl), F.hflip(edge)
        # return img, lbl
        return img, lbl, edge
...
class ExtToTensor(object):
    ...
    def __call__(self, pic, lbl, edge):
        """
        ...
        """
        if self.normalize:
            return F.to_tensor(pic), torch.from_numpy(np.array(lbl, dtype=self.target_type)), F.to_tensor(edge)
            ...
            return torch.from_numpy(np.array(pic, dtype=np.float32).transpose(2, 0, 1)), torch.from_numpy(
                np.array(lbl, dtype=self.target_type)), torch.from_numpy(
                np.array(edge, dtype=np.float32).transpose(2, 0, 1))
...
class ExtNormalize(object):
    ...
    def __call__(self, tensor, lbl, edge):
        """
        Args:
        ...
        return F.normalize(tensor, self.mean, self.std), lbl, F.normalize(edge, [0.5], [0.5]),
...
class ExtRandomCrop(object):
    ...
    def __call__(self, img, lbl, edge):
        ...
        if self.padding > 0:
            ...
            ###################################
            edge = F.pad(edge, self.padding)
            ###################################
        # pad the width if needed
        if self.pad_if_needed and img.size[0] < self.size[1]:
            img = F.pad(img, padding=int((1 + self.size[1] - img.size[0]) / 2))
            lbl = F.pad(lbl, padding=int((1 + self.size[1] - lbl.size[0]) / 2))
            edge = F.pad(edge, padding=int((1 + self.size[1] - lbl.size[0]) / 2))
        # pad the height if needed
        if self.pad_if_needed and img.size[1] < self.size[0]:
            img = F.pad(img, padding=int((1 + self.size[0] - img.size[1]) / 2))
            lbl = F.pad(lbl, padding=int((1 + self.size[0] - lbl.size[1]) / 2))
            edge = F.pad(edge, padding=int((1 + self.size[0] - lbl.size[1]) / 2))

        i, j, h, w = self.get_params(img, self.size)

        # return F.crop(img, i, j, h, w), F.crop(lbl, i, j, h, w)
        return F.crop(img, i, j, h, w), F.crop(lbl, i, j, h, w), F.crop(edge, i, j, h, w)
